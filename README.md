...
```sh
$ npm install @patrtorg/architecto-est
```
...
```javascript
import converter from "@patrtorg/architecto-est";
```

## Usage in Node.js(CommonJS):

```javascript
const {converter} = require("@patrtorg/architecto-est");
```
...
```html
<script src="/node_modules/@patrtorg/architecto-est/build/umd/index.js"></script>
```
This exposes the program on the window object:
```javascript
const javascriptBinaryConverter = window['@patrtorg/architecto-est']
...
```javascript
   ...
   const {HexStringConverter} = require('@patrtorg/architecto-est');
...
#### Binary to integer
 ```javascript
   const {BinaryConverter} = require('@patrtorg/architecto-est');
